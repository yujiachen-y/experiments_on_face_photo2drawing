...
class ProCDis(nn.Module):
    ...
    def __init__(self, input_dim, params, class_num):
        ...
        self.model = []
        # down sampling
        self.model += [Conv2dBlock(self.input_dim, self.dim, 7, 1, 2, norm=self.norm, activation=self.activ, pad_type=self.pad_type),]
        for i in range(2):
            self.model += [Conv2dBlock(self.dim, 2*self.dim, 4, 2, 1, norm=self.norm, activation=self.activ, pad_type=self.pad_type),]
            self.dim *= 2
        # self attention
        self.model += [
        ...
        self.fc = LinearBlock(self.dim, 1, norm=self.norm, activation=self.activ)

        self.y_vector = SpectralNorm(nn.Embedding(self.class_num, self.dim))

    def forward(self, x, y=None):
        x = self.model(x)
        x = torch.sum(x.view(1, self.dim, -1), dim=-1)
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, params):
        ...
        # style encoder
        self.enc_style = StyleEncoder(3, input_dim, dim, style_dim, mlp_dim, norm='none', activ=activ, pad_type=pad_type)
...
class StyleEncoder(nn.Module):
    def __init__(self, n_downsample, input_dim, dim, style_dim, mlp_dim, norm, activ, pad_type):
        super(StyleEncoder, self).__init__()
        style_count = 0
        self.down_samplers = nn.ModuleList()
        self.down_samplers.append(Conv2dBlock(input_dim, dim, 7, 1, 3, norm=norm, activation=activ, pad_type=pad_type))
        style_count += 2 * dim
        self.down_samplers.append(Conv2dBlock(dim, 2*dim, 4, 2, 1, norm=norm, activation=activ, pad_type=pad_type))
        style_count += 4 * dim
        dim *= 2
        self.down_samplers.append(nn.Sequential(Conv2dBlock(dim, 2*dim, 4, 2, 1, norm=norm, activation=activ, pad_type=pad_type), 
                                                SelfAttention(2*dim),))
        style_count += 4 * dim
        dim *= 2
        ...
        self.mlp = MLP(style_count+style_dim, style_dim, mlp_dim, 2)
    ...
    def forward(self, x):
        ...
        for down_sampler in self.down_samplers:
            x = down_sampler(x)
            style_code += [
                torch.mean(x.view(1, -1, x.shape[2]*x.shape[3]), dim=-1),
                torch.std(x.view(1, -1, x.shape[2]*x.shape[3]), dim=-1),
            ]
        x = self.model(x).view(1, -1)
        style_code = torch.cat(style_code + [x], dim=1)
...
class ContentEncoder(nn.Module):
    def __init__(self, n_downsample, n_res, input_dim, dim, norm, activ, pad_type):
        ...
        self.model += [
            SelfAttention(dim),
            ResBlock(dim, norm=norm, activation=activ, pad_type=pad_type),
            SelfAttention(dim),
        ]
        # residual blocks
        self.model += [ResBlocks(n_res-1, dim, norm=norm, activation=activ, pad_type=pad_type)]
        self.model = nn.Sequential(*self.model)
        self.output_dim = dim
...
class Decoder(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, res_norm='adain', activ='relu', pad_type='zero'):
        ...
        self.model += [
            SelfAttention(dim),
            ResBlock(dim, norm=res_norm, activation=activ, pad_type=pad_type),
            SelfAttention(dim),
        ]
        # upsampling blocks
