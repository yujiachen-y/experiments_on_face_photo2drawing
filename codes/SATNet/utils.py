"""
Modified from https://github.com/NVlabs/MUNIT/blob/master/utils.py
"""
import math
import os
import time
...
def get_data_loader_info(input_folder, batch_size, train, new_size=None,
                           height=256, width=256, num_workers=4, crop=True):
    transform_list = [transforms.ToTensor(),
                      transforms.Normalize((0.5, 0.5, 0.5),
                                           (0.5, 0.5, 0.5))]
...
def write_loss(iterations, trainer, train_writer, other_losses={}):
    members = [attr for attr in dir(trainer)\
               if not callable(getattr(trainer, attr)) and not attr.startswith("__") and ('loss' in attr or 'grad' in attr or 'nwd' in attr or 'acc' in attr or 'gamma' in attr)]
    if trainer is not None:
        other_losses.update(trainer.get_info())
    for m in members:
        train_writer.add_scalar(m, getattr(trainer, m), iterations + 1)
    for loss_name in other_losses:
        train_writer.add_scalar(loss_name, other_losses[loss_name], iterations + 1)
...
def sphereface_preprocess(batch):
    batch = downsample(batch) # (224, 192) -> (112, 96)
    (r, g, b) = torch.chunk(batch, 3, dim = 1)
    batch = torch.cat((b, g, r), dim = 1) # convert RGB to BGR
    batch = (batch + 1) * 255 * 0.5 # [-1, 1] -> [0, 255]
...
def get_scheduler(optimizer, hyperparameters, iterations=-1):
    ...
    elif hyperparameters['lr_policy'] == 'linear':
        scheduler = lr_scheduler.LambdaLR(optimizer, lambda epoch: 1 - max(0, epoch - hyperparameters['decay_step']) / hyperparameters['max_iter'])
    else:
